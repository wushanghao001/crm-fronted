<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-xl font-bold text-gray-800 dark:text-white">料号档案维护</h2>
        <p class="text-gray-500 dark:text-gray-400 mt-1">管理料号数据</p>
        <p class="text-sm text-amber-600 mt-1">提示：仅管理员可操作增删改，普通用户仅供查询</p>
      </div>
      <NButton v-if="authStore.hasPermission('materialcode:add')" type="primary" @click="handleAdd">
        <template #icon>
          <Add />
        </template>
        新增料号
        ...
        <table class="w-full">
          <thead>
            <tr class="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
              ...
              <th class="py-3 px-4 text-left text-sm font-medium text-gray-600 dark:text-gray-400">料号名称</th>
              <th class="py-3 px-4 text-left text-sm font-medium text-gray-600 dark:text-gray-400">料号编码</th>
            ...
            <tr v-for="item in dataList" :key="item.id" class="border-b border-gray-100 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">
              ...
              <td class="py-3 px-4">
                <div class="flex gap-2">
                  <NButton v-if="authStore.hasPermission('materialcode:edit')" size="small" text @click="handleEdit(item)">
                    <template #icon><Pencil /></template>
                    编辑
                  </NButton>
                  <NPopconfirm v-if="authStore.hasPermission('materialcode:delete')" @positive-click="handleDelete(item.id)">
                    <template #trigger>
    ...
    <NModal v-model:show="showModal" preset="card" :title="isEdit ? '编辑料号' : '新增料号'" style="width: 500px;">
      <NForm :model="formData" label-placement="left" label-width="100">
        <NFormItem label="料号名称" required>
          <NInput v-model:value="formData.name" placeholder="请输入料号名称" />
        </NFormItem>
        <NFormItem label="料号编码" required>
          <NInput v-model:value="formData.code" placeholder="请输入料号编码" />
        </NFormItem>
        <NFormItem label="状态">
...
import { getMaterialCodes, createMaterialCode, updateMaterialCode, deleteMaterialCode, batchDeleteMaterialCodes, MaterialCode } from '@/api/materialCode'
...
const dataList = ref<MaterialCode[]>([])
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await getMaterialCodes({
...
const handleEdit = (item: MaterialCode) => {
  isEdit.value = true
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value && editId.value) {
      await updateMaterialCode(editId.value, formData)
      message.success('更新成功')
    } else {
      await createMaterialCode(formData)
